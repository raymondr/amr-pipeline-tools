    ...
    with open(file) as f:
        while True:
            ...
            if not line.startswith('@'):
                ...
                contig = tokens[0]
                read = tokens[2]
                if contig in contig_to_read:
                    contig_to_read[contig].append(read)
                contig_to_read[contig] = [read]
...
def main():
    if len(sys.argv) != 5:
        print("Usage: test_scan_sam.py grouping.csv scanresults.scan align.sam scores.csv")
        sys.exit(0)
    ...
    tp = []
    nf = []
    contig_to_read = read_sam(sys.argv[3])
